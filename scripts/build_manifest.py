...
JEKYLL_CONFIG_PATH = ROOT / "_config.yml"
...
def main() -> int:
    ...
    write_jekyll_config(transcripts)
...
def write_jekyll_config(transcripts: list[dict]) -> None:
    include_paths: list[str] = []
    for item in transcripts:
        transcript_id = item.get("id", "")
        if not transcript_id.startswith("_"):
            continue
        include_paths.append(f"transcripts/{transcript_id}")
        for key in ("txt_path", "json_path", "srt_path", "vtt_path"):
            value = item.get(key)
            if value:
                include_paths.append(value)

    lines = [
        "# Generated by scripts/build_manifest.py",
        "# Keep underscore-prefixed transcript paths visible to GitHub Pages/Jekyll.",
        "include:",
    ]
    if include_paths:
        ...
        lines.append("  - .nojekyll")
    JEKYLL_CONFIG_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
